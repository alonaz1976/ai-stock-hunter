...
def compute_features(df: pd.DataFrame, intraday=False):
    ...
    x["volume_z"] = (x["Volume"] - vol_mean20) / vol_std20.replace(0, np.nan)
...
def score_row(r, min_turnover=3_000_000):
    ...
    m3, m5 = float(r.get("mom3", np.nan)), float(r.get("mom5", np.nan))
    momentum = 0
    ...
    vr = float(r.get("volume_ratio", np.nan))
    vz = float(r.get("volume_z", np.nan))
    va = float(r.get("vol_accel", np.nan))
    volume = 0
    if np.isfinite(vr):
        if vr >= 1.2: volume += 5
        if vr >= 1.5: volume += 4
    ...
    b = float(r.get("breakout20_pct", np.nan))
    breakout = 0
    if np.isfinite(b):
        if -3 <= b <= 0: breakout = 9
    ...
    cl = float(r.get("close_location", np.nan))
    candle = 0
    if np.isfinite(cl):
        if cl >= 0.75: candle = 6
    ...
    tv = float(r.get("turnover", np.nan))
    liq = 0
    if np.isfinite(tv):
...
def score_latest(feat: pd.DataFrame, min_turnover=3_000_000):
    ...
    return {
        "score": score,
        "price": float(r["Close"]),
        "volume_ratio": float(r.get("volume_ratio", np.nan)),
        "volume_z": float(r.get("volume_z", np.nan)),
        "rsi14": float(r.get("rsi14", np.nan)),
        "adx14": float(r.get("adx14", np.nan)),
        "atr_pct": float(r.get("atr_pct", np.nan)),
        "breakout20_pct": float(r.get("breakout20_pct", np.nan)),
        "daily_change_pct": float(r.get("ret1", np.nan))*100,
