...
def compute_features(df: pd.DataFrame, intraday=False):
    # Normalize OHLCV aggressively. A single malformed/nullable column should never
    # crash the whole scanner on Streamlit Cloud.
    x = df.copy()
    ...
    missing = [k for k in required if k not in x.columns]
    if missing:
        raise ValueError(f"Missing OHLCV columns: {missing}")
    for k in required:
        x[k] = pd.to_numeric(x[k], errors="coerce").astype("float64")
