...
def rsi(close, n=14):
    # Force a strict 1-D numeric Series and avoid pandas.replace in the division path.
    # This prevents the Streamlit Cloud TypeError seen with some yfinance/pandas builds.
    c = pd.Series(np.asarray(close, dtype="float64").reshape(-1), index=close.index, dtype="float64")
    d = c.diff()
    ...
    agv = ag.to_numpy(dtype="float64")
    alv = al.to_numpy(dtype="float64")
    rs = np.full(len(c), np.nan, dtype="float64")
    np.divide(agv, alv, out=rs, where=np.isfinite(alv) & (alv != 0.0))
    return pd.Series(100.0 - 100.0/(1.0 + rs), index=c.index, dtype="float64", name="rsi14")
