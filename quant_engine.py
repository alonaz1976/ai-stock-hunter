...
def rsi(close, n=14):
    """Robust Wilder RSI with a strict 1-D numeric input contract."""
    if isinstance(close, pd.DataFrame):
        if close.shape[1] < 1:
            return pd.Series(dtype="float64")
        close = close.iloc[:, 0]
    idx = getattr(close, "index", None)
    arr = np.asarray(close, dtype="float64").reshape(-1)
    if idx is None or len(idx) != len(arr):
        idx = pd.RangeIndex(len(arr))
    c = pd.Series(arr, index=idx, dtype="float64", name="Close")
    d = c.diff()
    ...
    rs_arr = np.divide(num, den, out=np.full_like(num, np.nan),
                       where=np.isfinite(den) & (den != 0))
    out = 100.0 - (100.0 / (1.0 + rs_arr))
    zero_loss = np.isfinite(den) & (den == 0) & np.isfinite(num) & (num > 0)
    out[zero_loss] = 100.0
    return pd.Series(out, index=idx, dtype="float64", name="RSI")
