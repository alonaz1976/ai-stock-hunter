...
def compute_features(df: pd.DataFrame, intraday=False):
    ...
    # Robust squeeze flags: perform the release transition on NumPy bool arrays.
    # This avoids pandas nullable/object boolean TypeErrors seen on Streamlit Cloud.
    squeeze_cmp=((bb_u < kc_u) & (bb_l > kc_l)).fillna(False)
    squeeze_arr=squeeze_cmp.to_numpy(dtype=bool, na_value=False)
    squeeze_prev_arr=np.concatenate(([False], squeeze_arr[:-1]))
    squeeze_release_arr=np.logical_and(squeeze_prev_arr, np.logical_not(squeeze_arr))
    x["squeeze"]=pd.Series(squeeze_arr, index=x.index, dtype=bool)
    x["squeeze_release"]=pd.Series(squeeze_release_arr, index=x.index, dtype=bool)
