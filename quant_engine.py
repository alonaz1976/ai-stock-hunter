...
def compute_features(df: pd.DataFrame, intraday=False):
    ...
    # Keep squeeze flags as plain boolean Series. Avoid unary ~ here because
    # some pandas/yfinance combinations can yield an object/nullable dtype.
    squeeze_now=((bb_u<kc_u)&(bb_l>kc_l)).fillna(False).astype(bool)
    squeeze_prev=squeeze_now.shift(1).fillna(False).astype(bool)
    x["squeeze"]=squeeze_now
    x["squeeze_release"]=squeeze_now.eq(False) & squeeze_prev
