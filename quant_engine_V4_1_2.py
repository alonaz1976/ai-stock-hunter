...
def _series1d(obj, index=None, name=None):
    """Convert Series/DataFrame/array-like input to one numeric 1-D pandas Series."""
    if isinstance(obj, pd.DataFrame):
        if obj.shape[1] == 0:
            # Preserve row count/index rather than constructing an empty ambiguous Series.
            idx = obj.index if index is None else index
            return pd.Series(np.full(len(idx), np.nan, dtype="float64"), index=idx, name=name)
        obj = obj.iloc[:, 0]
...
def _canonical_ohlcv(df: pd.DataFrame, ticker: str | None = None):
    """Return a strict 5-column float64 OHLCV frame.

    yfinance can return flat columns, duplicate labels, or MultiIndex columns in
    different level orders.  We never use boolean column masks here; instead we
    select the first matching physical column by position and rebuild a fresh
    DataFrame with canonical names.
    """
    ...
    for field in required:
        ...
        for pos, col in enumerate(cols):
            ...
            if any(x.lower() == field.lower() for x in pp):
                # Prefer a MultiIndex column that explicitly contains ticker.
    ...
    for field, pos in chosen.items():
        raw = df.iloc[:, pos]
        # iloc[:, pos] is guaranteed 1-D even if labels are duplicated.
        vals = pd.to_numeric(raw, errors="coerce").to_numpy(dtype="float64", na_value=np.nan)
        out[field] = vals
...
def rsi(close, n=14):
    """Robust Wilder RSI on a strict 1-D numeric series."""
    ...
    return pd.Series(out, index=c.index, name="RSI", dtype="float64")


def true_range(df):
    # Always return a 1-D float Series. This avoids pandas returning a DataFrame
    # when market-data providers deliver unusual/duplicate column metadata.
    high = pd.to_numeric(df["High"], errors="coerce").to_numpy(dtype="float64").reshape(-1)
    low = pd.to_numeric(df["Low"], errors="coerce").to_numpy(dtype="float64").reshape(-1)
    close = pd.to_numeric(df["Close"], errors="coerce").to_numpy(dtype="float64").reshape(-1)
    prev = np.empty_like(close)
    prev[0] = np.nan
    prev[1:] = close[:-1]
    tr = np.nanmax(np.vstack([high-low, np.abs(high-prev), np.abs(low-prev)]), axis=0)
    return pd.Series(tr, index=df.index, dtype="float64", name="TR")

def _wilder(series, n):
    # Robust Wilder smoothing with an explicit 1-D Series contract.
    s = pd.Series(np.asarray(series, dtype="float64").reshape(-1), index=series.index, dtype="float64")
    return s.ewm(alpha=1.0/float(n), adjust=False, min_periods=int(n)).mean()

def adx(df, n=14):
    high = pd.Series(np.asarray(df["High"], dtype="float64").reshape(-1), index=df.index)
    low = pd.Series(np.asarray(df["Low"], dtype="float64").reshape(-1), index=df.index)
    ...
    pdi = 100 * _wilder(pdm, n) / atr.replace(0, np.nan)
    mdi = 100 * _wilder(mdm, n) / atr.replace(0, np.nan)
...
def compute_features(df: pd.DataFrame, intraday=False):
    # Rebuild a canonical OHLCV frame first. This deliberately avoids operations
    # such as columns.duplicated()/boolean column masks that can fail on unusual
    # yfinance MultiIndex metadata in Streamlit Cloud.
    x = _canonical_ohlcv(df)
    if x is None or len(x) == 0:
        raise ValueError("No valid OHLCV rows after normalization")
    # Extract base fields by physical position so duplicate/odd column metadata
    # can never turn x["Close"] into a DataFrame.
    o = _series1d(x.iloc[:, 0], index=x.index, name="Open")
    hi = _series1d(x.iloc[:, 1], index=x.index, name="High")
    lo = _series1d(x.iloc[:, 2], index=x.index, name="Low")
    c = _series1d(x.iloc[:, 3], index=x.index, name="Close")
    v = _series1d(x.iloc[:, 4], index=x.index, name="Volume")
    x = pd.DataFrame({"Open": o, "High": hi, "Low": lo, "Close": c, "Volume": v}, index=x.index)
    ...
    tr=true_range(x); x["atr14"]=_wilder(tr,14); x["atr_pct"]=x["atr14"]/c*100; x["adx14"]=adx(x,14)
    ...
    x["bb_width"]=(bb_u-bb_l)/mid.replace(0,np.nan)
    # Squeeze flags are stored as plain 0/1 integers, not pandas booleans.
    # If this optional feature ever sees malformed data, it falls back to zero
    # rather than taking down Quant/Backtest/Scanner.
    squeeze_arr = np.zeros(len(x), dtype=np.int8)
    try:
        bu=np.asarray(bb_u, dtype="float64").reshape(-1)
        bl=np.asarray(bb_l, dtype="float64").reshape(-1)
        ku=np.asarray(kc_u, dtype="float64").reshape(-1)
        kl=np.asarray(kc_l, dtype="float64").reshape(-1)
        valid=np.isfinite(bu)&np.isfinite(bl)&np.isfinite(ku)&np.isfinite(kl)
        squeeze_arr[valid]=((bu[valid] < ku[valid]) & (bl[valid] > kl[valid])).astype(np.int8)
    except Exception:
        squeeze_arr[:] = 0
    squeeze_release_arr=np.zeros(len(x), dtype=np.int8)
    if len(x) > 1:
        squeeze_release_arr[1:]=((squeeze_arr[:-1] == 1) & (squeeze_arr[1:] == 0)).astype(np.int8)
    x["squeeze"]=squeeze_arr
    x["squeeze_release"]=squeeze_release_arr
    ...
    pv=((c.pct_change(5)<=0.02)&(x["obv"].diff(5)>0)).fillna(False)
    x["pv_divergence"]=pv.astype("int8")
...
def early_score_row(r):
    ...
    try: sq=float(r.get("squeeze",0) or 0) >= .5
    except Exception: sq=False
    try: rel=float(r.get("squeeze_release",0) or 0) >= .5
    except Exception: rel=False
    s=14 if rel else 9 if sq else 0; pts.append(("Bollinger/Keltner squeeze",s,14))
    ...
    try: div=float(r.get("pv_divergence",0) or 0) >= .5
    except Exception: div=False
    pts.append(("Price/Volume divergence",10 if div else 0,10))
...
def scan_universe(tickers,daily_period="6mo",use_hourly=True,hourly_period="1mo",horizon=5,target_pct=.06,min_turnover=3_000_000,buy_threshold=66,prefilter_top=30):
    """Two-stage scanner with per-ticker fault isolation.

    Stage 1 scans the full universe on daily data. Stage 2 adds 1h/15m,
    entry timing and backtest only to the strongest finalists. A malformed
    response for one ticker is skipped instead of crashing the entire scan.
    """
    stage1=[]
    for ticker in tickers:
        try:
            ...
            latest=score_latest(f,min_turnover)
        ...
        try:
            ...
            if use_hourly:
                ...
                if h is not None and len(h)>=30:
                    try:
                        hfeat=compute_features(h,True)
                        hl=score_latest(hfeat,0)
                        hs=hl["score"]; he=hl["early_score"]
                    except Exception:
                        hfeat=None; hs=he=np.nan
            ...
            rows.append({
                "Ticker":ticker,"Score":round(final,1),"EarlyScore":round(early,1),"PreScore":round(pre,1),
                "EntryScore":ent["entry_score"],"EntryStatus":ent["status"],
                "EntryLow":round(ent["zone_low"],4),"EntryHigh":round(ent["zone_high"],4),
                "Trigger":round(ent["trigger"],4),"Invalidation":round(ent["invalidation"],4),
                "Price":round(latest["price"],4),"DailyChangePct":round(latest["daily_change_pct"],2),
                ...
                "CMF20":round(latest["cmf20"],3) if np.isfinite(latest["cmf20"]) else np.nan,
                "HourlyScore":round(hs,1) if np.isfinite(hs) else np.nan,
                "EmpiricalHitRate":round(bt["hit_rate"]*100,1) if bt["n"] else np.nan,
                "BacktestN":bt["n"],"Confidence":bt["confidence_label"],"ConfidenceScore":bt["confidence"],
                "AvgReturn":round(bt["avg_return"]*100,2) if bt["n"] else np.nan,
                "MaxDrawdown":round(bt["max_drawdown"]*100,2) if bt["n"] else np.nan
            })
        except Exception:
            continue
