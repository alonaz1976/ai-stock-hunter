...
APP_VERSION = "5.8.0"
APP_BUILD_ID = "V580-ENTRY-CHART-20260914-A"
...
st.set_page_config(page_title="AI Stock Hunter — V5.8.0",page_icon="📈",layout="wide",initial_sidebar_state="collapsed")
st.markdown("""<style>
...
st.markdown("""<div class='hero'><span class='badge'>V5.8.0 • ENTRY VALIDATION • STABLE CHART • SCANNER PROGRESS</span><div class='hero-title'>📈 AI Stock Hunter<br>V5.8.0</div><div class='hero-sub'>Smart Money • Early Accumulation • Precision Entry • Walk-forward Backtest • Confidence Engine</div></div>""",unsafe_allow_html=True)
...
with tab1:
    ...
    st.caption("Chart controls are visual only. Zoom/pan state is preserved; use 5D/1M/3M/6M/1Y/MAX or double-click to reset. Pinch/scroll zoom is disabled on mobile to prevent the chart from disappearing.")
    ...
    if missing_engine_api:
        st.error("Quant engine is incompatible. Missing: " + ", ".join(missing_engine_api) + ". Upload all 4 files from the V5.8.0 ZIP.")
        st.stop()
    ...
    if st.button("Run V5.8 Dynamic analysis"):
...
with tab4:
    st.markdown("<div class='section'>V5.8 Walk-Forward Validation Lab</div>", unsafe_allow_html=True)
    ...
    if st.button("Run V5.8 Walk-Forward Validation", key="run_multi_validation"):
        tickers=[]
        ...
        if len(tickers)<3:
            ...
            bar=st.progress(0, text="Starting V5.8 walk-forward validation...")
            ...
            st.info("V5.8 gate: do not change live weights yet. A component should survive multiple unseen chronological folds and many stocks before V5.8 weight optimization.")
