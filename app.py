...
APP_VERSION = "5.9.6"
APP_BUILD_ID = "V596-LIVE-DECISION-20260915-A"
...
def add_market_and_opportunity(df):
    ...
    # Keep model setup separate from execution state: TRIGGER becomes LIVE only while that market is open.
    ...
    z.loc[z['PreviousSessionTrigger'] & z['MarketPhase'].eq('PRE-MARKET'),'LiveStage']='PRE-MARKET CONFIRM'
    z.loc[z['PreviousSessionTrigger'] & z['MarketPhase'].eq('CLOSED'),'LiveStage']='PREVIOUS SESSION TRIGGER'
    # Unified decision signal prevents a high Opportunity/Entry setup from being labelled AVOID.
    def decision(r):
        stg=str(r.get('OpportunityStage','WAIT')); pred=float(r.get('Prediction',0) or 0); conf=float(r.get('CalibrationConfidence',0) or 0)
    ...
    # TOP Score is the scanner ranking score; it blends opportunity with prediction and entry while retaining reliability.
    z['TopScore']=z.apply(lambda r: round(_clip100(.60*float(r.get('OpportunityScore',0) or 0)+.18*_clip100(r.get('Prediction',0))+.12*_clip100(r.get('EntryScore',0))+.10*_clip100(r.get('Reliability',0))),1),axis=1)
...
with tab2:
    ...
    only=st.checkbox("Show only BUY / WATCH",False)
