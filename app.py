...
APP_VERSION = "5.4.5"
APP_BUILD_ID = "V545-DYNAMIC-20260913-A"
...
# -----------------------------------------------------------------------------
# V5.4.5 robust dynamic-calibration fallback
...
st.set_page_config(page_title="AI Stock Hunter — V5.4.5",page_icon="📈",layout="wide",initial_sidebar_state="collapsed")
st.markdown("""<style>
...
st.markdown("""<div class='hero'><span class='badge'>V5.4.5 • DYNAMIC CALIBRATION • STATIC vs DYNAMIC • SIGNAL LIFT</span><div class='hero-title'>📈 AI Stock Hunter<br>V5.4.5</div><div class='hero-sub'>Smart Money • Early Accumulation • Precision Entry • Walk-forward Backtest • Confidence Engine</div></div>""",unsafe_allow_html=True)
...
with tab1:
    ...
    if missing_engine_api:
        st.error("Quant engine is incompatible. Missing: " + ", ".join(missing_engine_api) + ". Upload all 4 files from the V5.4.5 ZIP.")
        st.stop()
    ...
    if st.button("Run V5.4 Dynamic analysis"):
        ...
        with st.spinner("Running Quant + Early Prediction + Entry Engine..."):
            ...
            if d is None or len(d)<35: st.error("Not enough market data.")
                ...
                st.caption(f"A hit means price reached +{target}% within {horizon} trading days after a historical Quant score ≥ {buy_threshold}. For fewer than 12 signals, V5.4.5 shows hits/sample instead of highlighting a fragile percentage.")
