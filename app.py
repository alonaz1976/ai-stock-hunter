...
st.set_page_config(page_title="AI Stock Hunter — V5.3",page_icon="📈",layout="wide",initial_sidebar_state="collapsed")
st.markdown("""<style>
...
st.markdown("""<div class='hero'><span class='badge'>V5.3 • CALIBRATION LAB • RELIABLE BACKTEST DISPLAY</span><div class='hero-title'>📈 AI Stock Hunter<br>V5.3</div><div class='hero-sub'>Smart Money • Early Accumulation • Precision Entry • Walk-forward Backtest • Confidence Engine</div></div>""",unsafe_allow_html=True)
...
with tab1:
    ...
    if st.button("Run V5.3 analysis"):
        ...
        with st.spinner("Running Quant + Early Prediction + Entry Engine..."):
            ...
            if d is None or len(d)<35: st.error("Not enough market data.")
                ...
                st.caption(f"A hit means price reached +{target}% within {horizon} trading days after a historical Quant score ≥ {buy_threshold}. For fewer than 12 signals, V5.3 shows hits/sample instead of highlighting a fragile percentage.")
