...
APP_VERSION = "5.9.2"
APP_BUILD_ID = "V592-ADAPTIVE-RESEARCH-20260915-A"
...
st.markdown(f"""<div class='hero'><span class='badge'>V{APP_VERSION} • ADAPTIVE TARGET/HORIZON • MOVE SCORE • 60-STOCK RESEARCH • EXCEL</span><div class='hero-title'>📈 AI Stock Hunter<br>V{APP_VERSION}</div><div class='hero-sub'>Smart Money • Early Accumulation • Precision Entry • Adaptive Walk-forward Research</div></div>""",unsafe_allow_html=True)
...
HK_10="1196.HK,0700.HK,0005.HK,0883.HK,0941.HK,0002.HK,6160.HK,3750.HK,9633.HK,0823.HK"
VALIDATION_60=VALIDATION_50+","+HK_10
def safe(x,d=2):
        ...
        with st.spinner("Running Quant + Early Prediction + Entry Engine..."):
            ...
            if d is None or len(d)<35: st.error("Not enough market data.")
                ...
                analyze_summary=pd.DataFrame([{
                    ...
                    'HourlyConfirmation':ex.get('hourly_confirmation',np.nan) if ex else np.nan,'P15_5D':ex.get('p15_5d',np.nan) if ex else np.nan,
...
with tab2:
    ...
    universe_mode=st.radio("Universe",["Validation 60 (50 US + 10 HK)","Validation 50","Hong Kong 10","Broad 200","Core 26","Custom"],horizontal=True)
    if universe_mode.startswith("Validation 60"):
        default_universe=VALIDATION_60
    elif universe_mode == "Validation 50":
        default_universe=VALIDATION_50
    elif universe_mode == "Hong Kong 10":
        default_universe=HK_10
    ...
    prefilter_default=60 if universe_mode.startswith("Validation 60") else (50 if universe_mode == "Validation 50" else (10 if universe_mode == "Hong Kong 10" else 30))
    prefilter_top=st.slider("Deep-analysis finalists",10,60,prefilter_default,5,help="All tickers get the fast daily pass. Only this many finalists get 1h + 15m + entry timing + full backtest. Validation 50 defaults to all 50 so the full basket is ranked.")
    ...
    if st.button("Scan watchlist"):
        ...
        if res.empty:st.warning("No valid results returned.")
            ...
            res=res.sort_values(['Prediction','ExplosiveScore','DynamicQuant','DynamicEarly','EntryScore'],ascending=[False,False,False,False,False]).reset_index(drop=True)
    ...
    if 'v4res' in st.session_state:
        ...
        cols=['Rank','Ticker','Signal','Prediction','DynamicQuant','DynamicEarly','EntryScore','EntryStatus','ExplosiveScore','ExplosiveStage','HourlyConfirm','P5_5D','P10_5D','P15_5D','P15_5D_N','RobustRVOL','Retention','DryUp','ReExpansion','SignalLift','CalibrationConfidence','Price','VolumeRatio','RSI14','EmpiricalHitRate','BacktestN']
...
with tab7:
    st.markdown("<div class='section'>🧪 Adaptive 60-Stock Research</div>",unsafe_allow_html=True)
    st.caption("Runs the same causal research across Validation 60. This is research/validation, not a live buy signal. Lift, fold consistency and forward return are emphasized over raw hit rate.")
    ...
    if st.button("Run 60-Stock Research",key="run_60_research"):
        if not all(callable(x) for x in [historical_signal_timeline,threshold_optimization,adaptive_target_horizon,atr_target_validation]):
            st.error("V5.9.2 research functions are missing from quant_engine.py. Upload the matching V5.9.2 files.")
        else:
            research_rows=[]; opt_all=[]; atr_all=[]; timeline_samples=[]; skipped=[]
            rts=[x.strip() for x in VALIDATION_60.split(',') if x.strip()]
            ...
            for j,tkr in enumerate(rts):
                try:
                    ...
                    best=adaptive_target_horizon(both,int(r_min))
                    if best: research_rows.append({'Ticker':tkr,**best})
                    av=atr_target_validation(ff,threshold=int(best.get('Threshold',60)) if best else 60); av.insert(0,'Ticker',tkr); atr_all.append(av)
                    tl=historical_signal_timeline(ff).tail(260); tl.insert(0,'Ticker',tkr); timeline_samples.append(tl)
            ...
            sheets={'Adaptive Summary':summary,'Threshold Grid':pd.concat(opt_all,ignore_index=True) if opt_all else pd.DataFrame(),'ATR Validation':pd.concat(atr_all,ignore_index=True) if atr_all else pd.DataFrame(),'Historical Timeline':pd.concat(timeline_samples,ignore_index=True) if timeline_samples else pd.DataFrame(),'Skipped':pd.DataFrame(skipped)}
            st.session_state['excel_research60']=workbook_bytes(sheets,{'Tab':'Research 60','Version':APP_VERSION,'History':r_hist,'Universe':len(rts),'MinimumSignals':r_min,'Method':'Causal scores; future data labels only'})
    tab_download('excel_research60','⬇️ Download 60-Stock Research to Excel',f'AI_Stock_Hunter_V{APP_VERSION}_Research60')
