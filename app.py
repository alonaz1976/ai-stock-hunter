...
import threading, time as time_module, json, pickle, uuid, os, builtins
from plotly.subplots import make_subplots

APP_VERSION = "5.9.9"
APP_BUILD_ID = "V599-STARTUP-SAFE-20260915-C"
...
# Load the engine defensively. If anything in the environment/import fails,
# render the real error inside Streamlit instead of crashing before page setup.
_ENGINE_LOAD_ERROR = None
try:
    qe = _load_local_quant_engine()
except Exception as _engine_exc:
    qe = None
    _ENGINE_LOAD_ERROR = f"{type(_engine_exc).__name__}: {_engine_exc}"

def _qget(name, default=None):
    return getattr(qe, name, default) if qe is not None else default

fetch_ohlcv = _qget("fetch_ohlcv")
fetch_premarket_snapshots = _qget("fetch_premarket_snapshots", lambda tickers: {})
fetch_aftermarket_snapshots = _qget("fetch_aftermarket_snapshots", lambda tickers: {})
compute_features = _qget("compute_features")
score_latest = _qget("score_latest")
backtest_signal = _qget("backtest_signal")
scan_universe_dynamic = _qget("scan_universe_dynamic")
entry_timing = _qget("entry_timing")
score_row = _qget("score_row")
early_score_row = _qget("early_score_row")
dynamic_scores = _qget("dynamic_scores")
compare_static_dynamic_backtest = _qget("compare_static_dynamic_backtest")
explosive_latest = _qget("explosive_latest")
explosive_walkforward = _qget("explosive_walkforward")
historical_signal_timeline = _qget("historical_signal_timeline")
threshold_optimization = _qget("threshold_optimization")
adaptive_target_horizon = _qget("adaptive_target_horizon")
adaptive_target_horizon_v2 = _qget("adaptive_target_horizon_v2")
pre_move_study = _qget("pre_move_study")
topk_daily_validation = _qget("topk_daily_validation")
atr_target_validation = _qget("atr_target_validation")
hourly_confirmation = _qget("hourly_confirmation")
signal_timing_latest = _qget("signal_timing_latest")
acceleration_validation = _qget("acceleration_validation")
    ...
    for scan_i,ticker in enumerate(tickers,1):
        ...
        try:
            ...
            if d is None or len(d)<35:
                if progress_callback: progress_callback('skip',scan_i,len(tickers),ticker,'Insufficient daily data')
                continue
            ...
            if progress_callback: progress_callback('skip',scan_i,len(tickers),ticker,f'Stage 1: {type(e).__name__}: {e}')
            continue
...
if _ENGINE_LOAD_ERROR:
    st.error(f"Quant engine startup failed: {_ENGINE_LOAD_ERROR}")
    st.caption("V5.9.9 startup-safe mode caught the import error before a hard crash. Copy this exact message from the app or logs.")
    st.stop()
st.markdown("""<style>
...
def add_market_and_opportunity(df):
    ...
    def session_status(r):
        ...
        if phase=='PRE-MARKET':
            ...
            ch=r.get('PMChangePct',np.nan); chs=f' ({float(ch):+.2f}%)' if pd.notna(ch) else ''; return f'PRE-MARKET: {pmc}{chs} • PREVIOUS SESSION: {stage}'
        if phase=='AFTER-MARKET':
            ahc=str(r.get('AHConfirmation','N/A'))
            ch=r.get('AHChangePct',np.nan); chs=f' ({float(ch):+.2f}%)' if pd.notna(ch) else ''; return f'AFTER-MARKET: {ahc}{chs} • REGULAR SESSION: {stage}'
...
# V5.9.9 resilient scanner registry. Threads live in the Streamlit server process;
# status/results are also checkpointed to disk so a browser reconnect can recover them.
def _job_dir():
    # Lazy initialization avoids any filesystem side effect during app import/startup.
    d=Path(os.getenv('AI_STOCK_HUNTER_JOB_DIR','/tmp/ai_stock_hunter_v599'))
    d.mkdir(parents=True,exist_ok=True)
    return d

if not hasattr(builtins,'_AI_STOCK_HUNTER_JOBS'):
    builtins._AI_STOCK_HUNTER_JOBS={}
_SCAN_JOBS=builtins._AI_STOCK_HUNTER_JOBS

def _job_paths(job_id):
    d=_job_dir()
    return d/f'{job_id}.json', d/f'{job_id}.pkl'
def _write_job(job):
    meta={k:v for k,v in job.items() if k not in ('result','thread')}
    path,_=_job_paths(job['id']); tmp=path.with_suffix('.tmp')
    tmp.write_text(json.dumps(meta,default=str)); tmp.replace(path)
def _read_job(job_id):
    if job_id in _SCAN_JOBS: return _SCAN_JOBS[job_id]
    path,respath=_job_paths(job_id)
    if not path.exists(): return None
    try:
        j=json.loads(path.read_text())
        if respath.exists(): j['result']=pickle.loads(respath.read_bytes())
        _SCAN_JOBS[job_id]=j; return j
    except Exception:return None

def _run_scan_job(job_id,params):
    job=_SCAN_JOBS[job_id]; started=time_module.time(); skipped=[]
    def cb(stage,i,total,ticker,reason=None):
        if stage=='skip': skipped.append({'Ticker':ticker,'Reason':reason or 'Unknown'}); job['skipped']=skipped[-250:]
        else:
            # weighted progress: daily 55%, deep 45%
            pct=(55*i/max(1,total)) if stage=='stage1' else (55+45*i/max(1,total))
            job.update(stage=stage,current=ticker,done=int(i),stage_total=int(total),percent=round(min(100,pct),1))
        elapsed=time_module.time()-started; p=max(0.01,float(job.get('percent',0))/100); job['elapsed_sec']=round(elapsed,1); job['eta_sec']=round(elapsed*(1-p)/p,1) if p>0.02 else None
        _write_job(job)
    try:
        raw=_v544_scan_universe_dynamic(progress_callback=cb,**params)
        res=add_market_and_opportunity(raw) if raw is not None and not raw.empty else pd.DataFrame()
        if not res.empty: res=res.sort_values(['TopScore','OpportunityScore','ExplosiveScore','EntryScore'],ascending=False).reset_index(drop=True); res.insert(0,'Rank',range(1,len(res)+1))
        job.update(status='completed',percent=100.0,elapsed_sec=round(time_module.time()-started,1),eta_sec=0,finished=datetime.now().strftime('%Y-%m-%d %H:%M:%S'),successful=int(len(res)),skipped=skipped,result=res)
        _,rp=_job_paths(job_id); rp.write_bytes(pickle.dumps(res)); (_job_dir()/'last_completed.txt').write_text(job_id)
    except Exception as e: job.update(status='failed',error=f'{type(e).__name__}: {e}',elapsed_sec=round(time_module.time()-started,1))
    _write_job(job)

def _start_scan_job(params,total):
    jid=uuid.uuid4().hex[:12]; job={'id':jid,'status':'running','started':datetime.now().strftime('%Y-%m-%d %H:%M:%S'),'total':total,'done':0,'stage_total':total,'percent':0.0,'current':'—','elapsed_sec':0,'eta_sec':None,'skipped':[]}
    _SCAN_JOBS[jid]=job; _write_job(job); th=threading.Thread(target=_run_scan_job,args=(jid,params),daemon=True,name=f'scan-{jid}'); job['thread']=th; th.start(); return jid

def _fmt_duration(sec):
    if sec is None:return 'calculating…'
    sec=max(0,int(sec)); return f'{sec//60:02d}:{sec%60:02d}'

def _risk_reward_fields(r):
    try:
        lo=float(r.get('EntryZoneLow',np.nan)); hi=float(r.get('EntryZoneHigh',np.nan)); stop=float(r.get('Invalidation',np.nan)); t1=float(r.get('Target1',np.nan)); t2=float(r.get('Target2',np.nan)); mid=(lo+hi)/2
        risk=mid-stop
        if not all(np.isfinite(x) for x in [lo,hi,stop,t1,t2]) or risk<=0:return None
        return lo,hi,stop,t1,t2,(t1-mid)/risk,(t2-mid)/risk
    except:return None

def _stage_legend():
    st.markdown("""<div class='card'><b>Setup chronology — how to read the stages</b><div style='margin-top:10px;line-height:1.8'><span class='muted'>WAIT / COLD</span> → <span class='warn'>WATCH / BUILDING</span> → <span class='warn'>ARMED</span> → <span class='warn'>TRIGGER / TRIGGERED</span> → <span class='good'>LIVE TRIGGERED</span><br><span class='muted'>WAIT/COLD: no mature setup yet • WATCH/BUILDING: conditions are forming • ARMED: most conditions are ready • TRIGGERED: trigger conditions were reached in the measured session • LIVE TRIGGERED: confirmed while the regular market is open. Previous-session stages are historical context, not a live entry signal.</span></div></div>""",unsafe_allow_html=True)
...
with tab2:
    ...
    # Recover an in-progress or last completed server-side scan after reconnect/rerun.
    active_id=st.session_state.get('active_scan_job')
    if not active_id and (_job_dir()/'last_completed.txt').exists():
        try: active_id=(_job_dir()/'last_completed.txt').read_text().strip()
        except: active_id=None
    if st.button("Scan watchlist"):
        ts=[x.strip().upper() for x in tickers.split(',') if x.strip()]
        params=dict(tickers=ts,daily_period=sh,use_hourly=True,hourly_period="1mo",horizon=int(ho),target_pct=float(ta)/100,min_turnover=0,buy_threshold=int(th),prefilter_top=int(prefilter_top))
        active_id=_start_scan_job(params,len(ts)); st.session_state.active_scan_job=active_id
    job=_read_job(active_id) if active_id else None
    if job:
        if job.get('status')=='running':
            st.progress(int(job.get('percent',0)),text=f"Scanning {job.get('done',0)}/{job.get('stage_total',job.get('total',0))} • {job.get('percent',0):.0f}% • {job.get('current','—')}")
            st.caption(f"Elapsed {_fmt_duration(job.get('elapsed_sec'))} • Estimated remaining ~{_fmt_duration(job.get('eta_sec'))} • The server-side scan continues if this browser disconnects. Return/reload to reconnect to the same job.")
            if st.button("↻ Refresh scan status",key='refresh_scan_status'): st.rerun()
        elif job.get('status')=='failed': st.error('Scan failed: '+str(job.get('error','Unknown error')))
        elif job.get('status')=='completed':
            st.success(f"Completed • {job.get('successful',0)} results • {len(job.get('skipped',[]))} skipped • total time {_fmt_duration(job.get('elapsed_sec'))}")
            res=job.get('result',pd.DataFrame()).copy()
            # View filters are non-destructive; Last Completed Scan remains intact.
            if show_mode=='TOP OPPORTUNITIES': res=res[(res.TopScore>=68) | (res.OpportunityScore>=72)].reset_index(drop=True)
                ...
                actionable=((res.MarketPhase.eq('OPEN')) & res.LiveStage.eq('LIVE TRIGGERED')) | ((res.MarketPhase.eq('PRE-MARKET')) & res.PMConfirmation.eq('CONFIRMED')) | ((res.MarketPhase.eq('AFTER-MARKET')) & res.AHConfirmation.eq('CONFIRMED'))
            ...
            if 'Rank' in res: res['Rank']=range(1,len(res)+1)
            st.session_state.v4res=res; st.session_state.v4time=job.get('finished',job.get('started',''))
            st.session_state.v4skipped=pd.DataFrame(job.get('skipped',[]))
    if 'v4res' in st.session_state:
        ...
        for _,r in res.head(5).iterrows():
            status=str(getattr(r,'SessionStatus',getattr(r,'LiveStage','WATCH'))); status_class=cls(status)
            st.markdown(f"<div class='card'><div style='display:flex;justify-content:space-between'><div><b style='font-size:1.45rem'>🔥 #{int(r.Rank)} {r.Ticker}</b><div class='{status_class}'><b>{status}</b></div><div style='margin-top:6px'>TOP <b>{safe(getattr(r,'TopScore',np.nan),1)}</b> • Opportunity <b>{safe(getattr(r,'OpportunityScore',np.nan),1)}</b></div></div><div style='text-align:right'><span class='score'>{r.Prediction:.1f}</span><div class='muted'>Dynamic Prediction</div></div></div><hr style='border-color:#263249'><div class='muted'>Market {getattr(r,'Market','—')} • Market Rank #{int(getattr(r,'MarketRank',0))} • Move {safe(getattr(r,'MoveScore',np.nan),1)} • Explosive {safe(r.ExplosiveScore,1)} • Entry {safe(r.EntryScore,1)} • Reliability {safe(getattr(r,'Reliability',np.nan),1)} • RVOL {safe(r.VolumeRatio)}x • RSI {safe(r.RSI14,1)} • Hit {safe(r.EmpiricalHitRate,1)}% ({int(r.BacktestN)} signals) • Calibration {r.CalibrationConfidence:.0f}%</div></div>",unsafe_allow_html=True)
            if str(getattr(r,'LiveStage',''))=='LIVE TRIGGERED':
                lv=_risk_reward_fields(r)
                if lv:
                    lo,hi,stop,t1,t2,rr1,rr2=lv; st.markdown(f"<div class='card'><span class='good'><b>LIVE TRADE LEVELS — model/research levels</b></span><br>🟢 Entry Zone <b>{lo:.2f}–{hi:.2f}</b> • 🔴 Stop / Invalidation <b>{stop:.2f}</b> • 🎯 Target 1 <b>{t1:.2f}</b> • 🎯 Target 2 <b>{t2:.2f}</b><br>Risk/Reward: <b>1:{rr1:.2f}</b> to T1 • <b>1:{rr2:.2f}</b> to T2</div>",unsafe_allow_html=True)
        _stage_legend()
        if 'v4skipped' in st.session_state and not st.session_state.v4skipped.empty:
            with st.expander(f"Skipped stocks ({len(st.session_state.v4skipped)}) — reasons"):
                st.dataframe(st.session_state.v4skipped,use_container_width=True,hide_index=True)
        cols=['Rank','GlobalRank','MarketRank','Market','MarketPhase','SessionStatus','LiveStage','PreviousSessionTrigger','PMConfirmation','PMChangePct','PMVolume','PMVolumeStrength','AHConfirmation','AHChangePct','AHVolume','AHVolumeStrength','Sector','TopScore','OpportunityStage','OpportunityScore','Reliability','TimingStage','MoveScore','ExplosiveScore','EntryScore','EntryZoneLow','EntryZoneHigh','BreakoutTrigger','Invalidation','Target1','Target2','Accel1D','Accel2D','Accel3D','Rising3D','HourlyConfirm','Signal','Prediction','DynamicQuant','DynamicEarly','EntryStatus','ExplosiveStage','P5_5D','P10_5D','P15_5D','P15_5D_N','RobustRVOL','Retention','DryUp','ReExpansion','SignalLift','CalibrationConfidence','Price','VolumeRatio','RSI14','EmpiricalHitRate','BacktestN']
