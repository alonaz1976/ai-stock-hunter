...
import threading, time as time_module, uuid
from plotly.subplots import make_subplots

APP_VERSION = "5.9.9"
APP_BUILD_ID = "V599-CACHED-WORKER-20260915-D"
...
if _ENGINE_LOAD_ERROR:
    ...
    st.caption("V5.9.9 diagnostic mode caught the quant-engine import error. Copy this exact message from the app or logs.")
    st.stop()
st.markdown("""<style>
...
# V5.9.9 resilient scanner manager.
# The manager is a Streamlit cached server resource: no filesystem/pickle/builtins
# work happens during startup. Worker threads never call Streamlit APIs.
class _ScanManager:
    def __init__(self):
        self.jobs={}
...
_SCAN_MANAGER=_get_scan_manager()

def _write_job(job):
    # In-memory checkpoint; guarded because scanner thread and UI reruns can overlap.
    with _SCAN_MANAGER.lock:
        _SCAN_MANAGER.jobs[job['id']]=job

def _read_job(job_id):
    if not job_id:
        return None
    with _SCAN_MANAGER.lock:
        return _SCAN_MANAGER.jobs.get(job_id)

def _run_scan_job(job_id,params):
    with _SCAN_MANAGER.lock:
        job=_SCAN_MANAGER.jobs[job_id]
    started=time_module.time(); skipped=[]
    def cb(stage,i,total,ticker,reason=None):
        with _SCAN_MANAGER.lock:
            if stage=='skip':
                skipped.append({'Ticker':ticker,'Reason':reason or 'Unknown'})
                job['skipped']=list(skipped[-250:])
            else:
            ...
            p=max(0.01,float(job.get('percent',0))/100)
            job['elapsed_sec']=round(elapsed,1)
            job['eta_sec']=round(elapsed*(1-p)/p,1) if p>0.02 else None
    try:
        ...
        with _SCAN_MANAGER.lock:
            job.update(status='completed',percent=100.0,elapsed_sec=round(time_module.time()-started,1),eta_sec=0,finished=datetime.now().strftime('%Y-%m-%d %H:%M:%S'),successful=int(len(res)),skipped=list(skipped),result=res)
            _SCAN_MANAGER.last_completed=job_id
    except Exception as e:
        with _SCAN_MANAGER.lock:
            job.update(status='failed',error=f'{type(e).__name__}: {e}',elapsed_sec=round(time_module.time()-started,1))

def _start_scan_job(params,total):
    jid=uuid.uuid4().hex[:12]
    job={'id':jid,'status':'running','started':datetime.now().strftime('%Y-%m-%d %H:%M:%S'),'total':total,'done':0,'stage_total':total,'percent':0.0,'current':'—','elapsed_sec':0,'eta_sec':None,'skipped':[]}
    with _SCAN_MANAGER.lock:
        _SCAN_MANAGER.jobs[jid]=job
    th=threading.Thread(target=_run_scan_job,args=(jid,params),daemon=True,name=f'scan-{jid}')
    job['thread']=th
    th.start()
    return jid
...
with tab2:
    ...
    if not active_id:
        active_id=_SCAN_MANAGER.last_completed
