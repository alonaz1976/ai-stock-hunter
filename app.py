...
APP_VERSION = "5.5.0"
APP_BUILD_ID = "V550-MULTISTOCK-20260914-A"
...
st.set_page_config(page_title="AI Stock Hunter — V5.5.0",page_icon="📈",layout="wide",initial_sidebar_state="collapsed")
st.markdown("""<style>
...
st.markdown("""<div class='hero'><span class='badge'>V5.5.0 • DYNAMIC CALIBRATION • STATIC vs DYNAMIC • SIGNAL LIFT</span><div class='hero-title'>📈 AI Stock Hunter<br>V5.5.0</div><div class='hero-sub'>Smart Money • Early Accumulation • Precision Entry • Walk-forward Backtest • Confidence Engine</div></div>""",unsafe_allow_html=True)
...
with tab4:
    st.markdown("<div class='section'>Multi-Stock Validation Lab</div>", unsafe_allow_html=True)
    st.caption("Research mode only. It runs the same Early + Quant calibration across many stocks and checks which components repeat consistently. It does NOT change the live model weights.")
    default_universe = "NVDA, AMD, MU, AVGO, NVMI, MSFT, GOOGL, META, AMZN, AAPL, JPM, BAC, XOM, CVX, LLY, UNH, CAT, DE, COST, WMT, KO, PG, DIS, BA, PLTR"
    universe_text = st.text_area("Validation universe", default_universe, height=120, help="Comma-separated Yahoo Finance tickers. Start with the diversified 25-stock basket.")
    vc1,vc2,vc3 = st.columns(3)
    vhist = vc1.selectbox("Validation history", ["6mo","1y"], 0, key="v_hist")
    vtarget = vc2.selectbox("Validation target %", [3,5,6,8], 0, key="v_target")
    min_events = vc3.selectbox("Minimum events / stock", [3,5,8,10], 1, key="v_min_events")
    st.caption("Recommended first run: 25 stocks • 6M • +3% target • minimum 5 events. Stocks with too little data are reported separately.")

    if st.button("Run Multi-Stock Validation", key="run_multi_validation"):
        tickers=[]
        ...
        tickers=tickers[:50]
        if len(tickers)<3:
            st.error("Enter at least 3 tickers for a cross-stock validation.")
        else:
            early_rows=[]; quant_rows=[]; stock_rows=[]; skipped=[]
            bar=st.progress(0, text="Starting multi-stock validation...")
            for idx,t in enumerate(tickers,1):
                bar.progress(int(100*idx/len(tickers)), text=f"Calibrating {t} ({idx}/{len(tickers)})")
                try:
                    d=fetch_ohlcv(t,vhist,"1d")
                    if d is None or len(d)<35:
                        skipped.append((t,"not enough price history")); continue
                    f=compute_features(d)
                    ds=dynamic_scores(f,float(vtarget)/100,0)
                    ec=ds.get('early_calibration',{}); qc=ds.get('quant_calibration',{})
                    ev=int(min(ec.get('events',0) or 0,qc.get('events',0) or 0))
                    if ev<int(min_events):
                        skipped.append((t,f"only {ev} target events")); continue
                    stock_rows.append({'Ticker':t,'Events':ev,'Early confidence %':round(float(ec.get('confidence',0)),1),'Quant confidence %':round(float(qc.get('confidence',0)),1)})
                    for kind,cal,dest in [('Early',ec,early_rows),('Quant',qc,quant_rows)]:
                        tb=cal.get('table',pd.DataFrame())
                        if tb is None or tb.empty: continue
                        for _,r in tb.iterrows():
                            dest.append({'Ticker':t,'Component':r['Component'],'Lift x':float(r.get('Lift x',np.nan)),'Coverage %':float(r.get('Coverage %',np.nan)),'Events':ev,'Stability %':float(r.get('Stability %',np.nan))})
                except Exception as e:
                    skipped.append((t,"calculation/data error"))
            bar.empty()
            ...
            def aggregate_component(rows):
                ...
                for comp,g in z.groupby('Component'):
                    lifts=pd.to_numeric(g['Lift x'],errors='coerce').dropna()
                    cov=pd.to_numeric(g['Coverage %'],errors='coerce').dropna()
                    stab=pd.to_numeric(g['Stability %'],errors='coerce').dropna()
                    valid=len(lifts); pos=int((lifts>1.0).sum()) if valid else 0
                    pos_rate=100*pos/valid if valid else 0
                    med=float(lifts.median()) if valid else np.nan
                    mean=float(lifts.mean()) if valid else np.nan
                    mx=float(lifts.max()) if valid else np.nan
                    if valid>=5 and med>=1.05 and pos_rate>=65:
                        verdict='CONSISTENT WINNER'
                    elif valid>=5 and (med<0.95 or pos_rate<=40):
                        verdict='WEAK / NEGATIVE'
                    elif valid>=5 and mx>=1.25 and pos_rate<65:
                        verdict='STOCK SPECIFIC'
                    else:
                        verdict='MIXED / NEUTRAL'
                    out.append({'Component':comp,'Median Lift x':med,'Mean Lift x':mean,'Avg Coverage %':float(cov.mean()) if len(cov) else np.nan,'Positive stocks':f'{pos}/{valid}','Positive %':pos_rate,'Avg Stability %':float(stab.mean()) if len(stab) else np.nan,'Total events':int(g.groupby('Ticker')['Events'].first().sum()),'Max Lift x':mx,'Verdict':verdict})
                a=pd.DataFrame(out)
                rank={'CONSISTENT WINNER':0,'STOCK SPECIFIC':1,'MIXED / NEUTRAL':2,'WEAK / NEGATIVE':3}
                a['_rank']=a['Verdict'].map(rank).fillna(9)
                return a.sort_values(['_rank','Median Lift x','Positive %'],ascending=[True,False,False]).drop(columns='_rank').reset_index(drop=True)

            ea=aggregate_component(early_rows); qa=aggregate_component(quant_rows)
            total_events=sum(r['Events'] for r in stock_rows)
            m1,m2,m3=st.columns(3)
            m1.metric("Stocks validated",f"{len(stock_rows)}/{len(tickers)}")
            m2.metric("Total target events",total_events)
            m3.metric("Target",f"+{vtarget}%")
            st.markdown("<div class='section'>Early — cross-stock consistency</div>",unsafe_allow_html=True)
            if not ea.empty:
                show=ea.copy()
                for c in ['Median Lift x','Mean Lift x','Avg Coverage %','Positive %','Avg Stability %','Max Lift x']:
                    show[c]=pd.to_numeric(show[c],errors='coerce').round(2)
                st.dataframe(show,use_container_width=True,hide_index=True)
                winners=ea[ea['Verdict']=='CONSISTENT WINNER']
                if not winners.empty:
                    names=', '.join(winners['Component'].head(4).tolist())
                    st.success(f"Early components that repeat best across this basket: {names}")
            st.markdown("<div class='section'>Quant — cross-stock consistency</div>",unsafe_allow_html=True)
            if not qa.empty:
                show=qa.copy()
                for c in ['Median Lift x','Mean Lift x','Avg Coverage %','Positive %','Avg Stability %','Max Lift x']:
                    show[c]=pd.to_numeric(show[c],errors='coerce').round(2)
                st.dataframe(show,use_container_width=True,hide_index=True)
                winners=qa[qa['Verdict']=='CONSISTENT WINNER']
                if not winners.empty:
                    names=', '.join(winners['Component'].head(4).tolist())
                    st.success(f"Quant components that repeat best across this basket: {names}")
            ...
            st.info("Next gate: do not change base weights yet. First look for components that remain positive across many stocks; then V5.6 will test those findings out-of-sample.")
