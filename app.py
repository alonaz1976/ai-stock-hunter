...
from quant_engine import fetch_ohlcv, compute_features, score_latest, backtest_signal, scan_universe_dynamic, entry_timing, score_row, early_score_row, dynamic_scores, compare_static_dynamic_backtest
...
st.set_page_config(page_title="AI Stock Hunter — V5.4.0",page_icon="📈",layout="wide",initial_sidebar_state="collapsed")
st.markdown("""<style>
...
st.markdown("""<div class='hero'><span class='badge'>V5.4.0 • DYNAMIC CALIBRATION • STATIC vs DYNAMIC • SIGNAL LIFT</span><div class='hero-title'>📈 AI Stock Hunter<br>V5.4.0</div><div class='hero-sub'>Smart Money • Early Accumulation • Precision Entry • Walk-forward Backtest • Confidence Engine</div></div>""",unsafe_allow_html=True)
...
with tab1:
    ...
    required_engine_api = ["fetch_ohlcv", "compute_features", "score_latest", "backtest_signal", "scan_universe_dynamic", "entry_timing", "score_row", "early_score_row"]
    ...
    if missing_engine_api:
        st.error("Quant engine is incompatible. Missing: " + ", ".join(missing_engine_api) + ". Upload all 4 files from the V5.4.0 ZIP.")
        st.stop()
    elif engine_ver is not None and str(engine_ver) != "5.4.0":
        st.warning(f"Version mismatch detected: app V5.4.0 / engine {engine_ver}. Upload all 4 files from the same V5.4.0 ZIP before trusting the results.")
    elif engine_ver is not None:
        st.caption("✓ App and quant engine synced: V5.4.0")
    else:
        st.caption("✓ Quant engine compatibility check passed")
    if st.button("Run V5.4 Dynamic analysis"):
        ...
        with st.spinner("Running Quant + Early Prediction + Entry Engine..."):
            ...
            if d is None or len(d)<35: st.error("Not enough market data.")
                ...
                a,b,c,d1,e=st.columns(5); a.metric("Dynamic Quant",f"{dyn['dynamic_quant']:.1f}",delta=f"Static {dyn['static_quant']:.1f}"); b.metric("Dynamic Early",f"{dyn['dynamic_early']:.1f}",delta=f"Static {dyn['static_early']:.1f}"); c.metric("Final Prediction",f"{dyn['final_prediction']:.1f}"); d1.metric("Entry Score",f"{ent['entry_score']:.1f}"); e.metric("Backtest",bt_value)
                ...
                st.caption(f"A hit means price reached +{target}% within {horizon} trading days after a historical Quant score ≥ {buy_threshold}. For fewer than 12 signals, V5.4.0 shows hits/sample instead of highlighting a fragile percentage.")
...
with tab2:
    ...
    if st.button("Scan watchlist"):
        ...
        with st.spinner("Running V5.4 Dynamic Calibration scanner..."): res=scan_universe_dynamic(ts,sh,True,"1mo",int(ho),float(ta)/100,0,int(th),int(prefilter_top))
