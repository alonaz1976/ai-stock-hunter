...
APP_VERSION = "5.9.3"
APP_BUILD_ID = "V593-ADAPTIVE-RESEARCH-20260915-A"
...
st.markdown(f"""<div class='hero'><span class='badge'>V{APP_VERSION} • RELIABILITY V2 • PRE-MOVE • TOP-K • OUT-OF-SAMPLE • EXCEL</span><div class='hero-title'>📈 AI Stock Hunter<br>V{APP_VERSION}</div><div class='hero-sub'>Smart Money • Reliability-Hardened Adaptive Research • Precision Entry • Walk-forward Validation</div></div>""",unsafe_allow_html=True)
...
with tab7:
    ...
    st.caption("Runs causal research across Validation 60. V5.9.3 hardens Best Setup with Reliability Score, pre-move analysis and cross-universe Top-K validation.")
    ...
    if st.button("Run 60-Stock Research",key="run_60_research"):
        if not all(callable(x) for x in [historical_signal_timeline,threshold_optimization,adaptive_target_horizon_v2,atr_target_validation,pre_move_study,topk_daily_validation]):
            ...
            research_rows=[]; opt_all=[]; atr_all=[]; timeline_samples=[]; premove_all=[]; skipped=[]
            ...
            for j,tkr in enumerate(rts):
                try:
                    ...
                    tl_full=historical_signal_timeline(ff); pm=pre_move_study(tl_full); pm.insert(0,'Ticker',tkr); premove_all.append(pm); tl=tl_full.tail(260); tl.insert(0,'Ticker',tkr); timeline_samples.append(tl)
