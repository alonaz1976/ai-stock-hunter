...
APP_VERSION = "5.9.5"
APP_BUILD_ID = "V595-OPPORTUNITY-150-MOBILE-20260915-A"
...
def add_market_and_opportunity(df):
    ...
    z=pd.concat([z,vals],axis=1).sort_values(['OpportunityScore','ExplosiveScore','EntryScore'],ascending=False).reset_index(drop=True)
    z['GlobalRank']=np.arange(1,len(z)+1)
    z['MarketRank']=z.groupby('Market')['OpportunityScore'].rank(method='first',ascending=False).astype(int)
    return z
...
with tab2:
    ...
    if st.button("Scan watchlist"):
        ...
        if res.empty:st.warning("No valid results returned.")
            ...
            res=res.sort_values(['OpportunityScore','ExplosiveScore','EntryScore'],ascending=False).reset_index(drop=True)
