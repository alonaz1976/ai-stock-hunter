...
st.set_page_config(page_title="AI Stock Hunter — Quant V4.1",page_icon="📈",layout="wide",initial_sidebar_state="collapsed")
st.markdown("""<style>
...
st.markdown("""<div class='hero'><span class='badge'>QUANT V4.1 • EARLY PREDICTION • ENTRY ENGINE</span><div class='hero-title'>📈 AI Stock Hunter<br>Quant V4.1</div><div class='hero-sub'>Smart Money • Early Accumulation • Precision Entry • Walk-forward Backtest • Confidence Engine</div></div>""",unsafe_allow_html=True)
...
with tab1:
    ...
    if st.button("Run V4.1 analysis"):
...
with tab2:
    ...
    if st.button("Scan watchlist"):
        ...
        with st.spinner("Scanning Quant V4.1 universe..."): res=scan_universe(ts,sh,True,"1mo",int(ho),float(ta)/100,0,int(th),int(prefilter_top))
