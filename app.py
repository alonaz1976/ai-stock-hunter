print("[TRACE 00] app.py start", flush=True)
...
print("[TRACE 01] imports complete", flush=True)

APP_VERSION = "5.9.9"
APP_BUILD_ID = "V599-STABLE-SYNC-20260915-E"
...
print(f"[TRACE 02] engine load complete error={_ENGINE_LOAD_ERROR}", flush=True)
...
print("[TRACE 03] before set_page_config", flush=True)
...
if _ENGINE_LOAD_ERROR:
    ...
    st.stop()
print("[TRACE 04] page_config complete", flush=True)
st.markdown("""<style>
...
# V5.9.9 stable scanner helpers.
# Background worker/cache-resource startup logic was removed after diagnostic isolation.
...
print("[TRACE 05] hero/css/universe setup complete", flush=True)
tab1,tab2,tab3,tab4,tab5,tab6,tab7=st.tabs(["◉ Analyze","⌁ Scanner","▦ Backtest","◎ Validate","↗ Entry Validation","⚡ Explosive Lab","🧪 Research 60"])
print("[TRACE 06] tabs created", flush=True)
with tab1:
    print("[TRACE 07] entering tab1", flush=True)
...
print("[TRACE 08] tab1 complete", flush=True)
with tab2:
    print("[TRACE 09] entering tab2", flush=True)
    ...
    if st.button("Scan watchlist"):
        ...
        def _scanner_progress(stage,i,total,ticker_name,reason=None):
            if stage=='skip':
                skipped.append({'Ticker':ticker_name,'Reason':reason or 'Unknown'})
                return
            pct=(55*i/max(1,total)) if stage=='stage1' else (55+45*i/max(1,total))
            pct=max(0,min(100,float(pct)))
            elapsed=max(0.0,time_module.time()-started)
            p=max(0.01,pct/100.0)
            eta=(elapsed*(1-p)/p) if pct>=3 else None
            phase='Stage 1/2 • Daily scan' if stage=='stage1' else 'Stage 2/2 • Deep analysis'
            scan_bar.progress(int(pct),text=f"{phase} {i}/{total} • {ticker_name} • elapsed {_fmt_duration(elapsed)} • ETA ~{_fmt_duration(eta)}")
        ...
        scan_bar.progress(100,text=f"Completed • total time {_fmt_duration(elapsed)}")
        if res is None or res.empty:
            st.warning("No valid results returned.")
            st.session_state.v4skipped=pd.DataFrame(skipped)
        else:
            ...
            st.session_state.v4res_full=res.copy()
            st.session_state.v4time=datetime.now().strftime('%Y-%m-%d %H:%M:%S')
            st.session_state.v4duration=elapsed
            st.session_state.v4skipped=pd.DataFrame(skipped)
    if 'v4res_full' in st.session_state:
        res=st.session_state.v4res_full.copy()
        ...
        st.session_state.v4res=res
        st.success(f"Last Completed Scan • {len(st.session_state.v4res_full)} results • {len(st.session_state.get('v4skipped',pd.DataFrame()))} skipped • total time {_fmt_duration(st.session_state.get('v4duration'))}")
...
print("[TRACE 10] tab2 complete", flush=True)
with tab3:
    print("[TRACE 11] entering tab3", flush=True)
...
print("[TRACE 12] tab3 complete", flush=True)
with tab4:
    print("[TRACE 13] entering tab4", flush=True)
...
print("[TRACE 14] tab4 complete", flush=True)
with tab5:
    print("[TRACE 15] entering tab5", flush=True)
...
print("[TRACE 16] tab5 complete", flush=True)
with tab6:
    print("[TRACE 17] entering tab6", flush=True)
...
print("[TRACE 18] tab6 complete", flush=True)
with tab7:
    print("[TRACE 19] entering tab7", flush=True)
...
print("[TRACE 20] app.py reached end", flush=True)
