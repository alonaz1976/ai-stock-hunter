...
st.markdown(f"""<div class='hero'><span class='badge'>V{APP_VERSION} • SIGNAL TIMING • 1D/2D/3D ACCELERATION • HOURLY TRIGGER • RELIABILITY • EXCEL</span><div class='hero-title'>📈 AI Stock Hunter<br>V{APP_VERSION}</div><div class='hero-sub'>Smart Money • Reliability-Hardened Adaptive Research • Precision Entry • Walk-forward Validation</div></div>""",unsafe_allow_html=True)
...
with tab2:
    ...
    st.caption(f"Stage 1 scans {len([x for x in tickers.split(chr(44)) if x.strip()])} tickers on daily data. Stage 2 performs deep intraday analysis on the top {prefilter_top}. Results are ranked by Signal Timing Stage, Explosive Score, 3D Acceleration, Entry Score and Dynamic Prediction. Acceleration is a score change, not a price %.")
    ...
    if st.button("Scan watchlist"):
        ...
        if res.empty:st.warning("No valid results returned.")
            ...
            res['_TimingRank']=res.get('TimingStage',pd.Series(['—']*len(res))).map(stage_rank).fillna(0)
            res=res.sort_values(['_TimingRank','ExplosiveScore','Accel3D','EntryScore','Prediction'],ascending=[False,False,False,False,False]).drop(columns=['_TimingRank']).reset_index(drop=True)
...
with tab7:
    ...
    st.caption("Runs causal research across Validation 60. V5.9.4 adds causal 1D/2D/3D score acceleration, trajectory consistency and Signal Timing validation on top of Reliability and Top-K.")
