...
APP_VERSION = "5.6.0"
APP_BUILD_ID = "V560-VALIDATION-20260914-A"
...
st.set_page_config(page_title="AI Stock Hunter — V5.6.0",page_icon="📈",layout="wide",initial_sidebar_state="collapsed")
st.markdown("""<style>
...
st.markdown("""<div class='hero'><span class='badge'>V5.6.0 • OUT-OF-SAMPLE VALIDATION • STATIC vs DYNAMIC • SIGNAL LIFT</span><div class='hero-title'>📈 AI Stock Hunter<br>V5.6.0</div><div class='hero-sub'>Smart Money • Early Accumulation • Precision Entry • Walk-forward Backtest • Confidence Engine</div></div>""",unsafe_allow_html=True)
...
with tab2:
    ...
    if st.button("Scan watchlist"):
        ...
        with st.spinner("Running V5.5 Dynamic Calibration scanner..."): res=scan_universe_dynamic(ts,sh,True,"1mo",int(ho),float(ta)/100,0,int(th),int(prefilter_top))
...
with tab4:
    st.markdown("<div class='section'>V5.6 Validation Basket — Out-of-Sample</div>", unsafe_allow_html=True)
    st.caption("Discovery and validation are separated. The first 70% of each stock history finds candidate components; the final 30% checks whether they still work on unseen later data. Live model weights are NOT changed.")
    default_universe = "NVDA,AMD,MU,AVGO,NVMI,TSM,QCOM,ARM,INTC,AMAT,MSFT,GOOGL,META,AMZN,AAPL,ORCL,CRM,ADBE,NOW,PLTR,JPM,BAC,GS,MS,V,MA,CAT,DE,GE,HON,RTX,BA,LLY,UNH,JNJ,MRK,ABBV,GILD,COST,WMT,KO,PG,MCD,NKE,XOM,CVX,NEE,DIS,UBER,NFLX"
    universe_text = st.text_area("Validation universe", default_universe, height=145, help="50-stock diversified basket. You can edit the tickers before running.")
    vc1,vc2,vc3 = st.columns(3)
    vhist = vc1.selectbox("Validation history", ["1y","2y"], 0, key="v_hist")
    vtarget = vc2.selectbox("Validation target %", [3,5,6,8], 0, key="v_target")
    min_events = vc3.selectbox("Minimum discovery events / stock", [3,5,8,10], 0, key="v_min_events")
    st.caption("Recommended: 50 stocks • 1Y • +3% target. 70% discovery / 30% unseen validation. A stock is skipped only with an explicit reason.")

    if st.button("Run V5.6 Out-of-Sample Validation", key="run_multi_validation"):
        tickers=[]
        ...
        if len(tickers)<3:
            ...
            train_early=[]; train_quant=[]; test_early=[]; test_quant=[]; stock_rows=[]; skipped=[]
            bar=st.progress(0, text="Starting V5.6 validation...")

            def collect_period(feat, ticker, kind, target, dest, phase):
                cal=_v544_calibrate_components(feat,kind,target,(1,2,3),0)
                tb=cal.get('table',pd.DataFrame())
                ev=int(cal.get('events',0) or 0)
                if tb is not None and not tb.empty:
                    for _,r in tb.iterrows():
                        dest.append({'Ticker':ticker,'Component':r['Component'],'Lift x':float(r.get('Lift x',np.nan)),'Coverage %':float(r.get('Coverage %',np.nan)),'Events':ev,'Stability %':float(r.get('Stability %',np.nan)),'Phase':phase})
                return ev

            for idx,t in enumerate(tickers,1):
                bar.progress(int(100*idx/len(tickers)), text=f"Validating {t} ({idx}/{len(tickers)})")
                try:
                    d=fetch_ohlcv(t,vhist,"1d")
                    if d is None:
                        skipped.append((t,"no price data returned")); continue
                    if len(d)<100:
                        skipped.append((t,f"only {len(d)} daily rows; need at least 100")); continue
                    f=compute_features(d).dropna(subset=['Close']).copy()
                    cut=max(70,int(len(f)*0.70))
                    if len(f)-cut<30:
                        skipped.append((t,"validation window too short")); continue
                    train=f.iloc[:cut].copy(); test=f.iloc[cut:].copy()
                    te=collect_period(train,t,'early',float(vtarget)/100,train_early,'Discovery')
                    tq=collect_period(train,t,'quant',float(vtarget)/100,train_quant,'Discovery')
                    ve=collect_period(test,t,'early',float(vtarget)/100,test_early,'Validation')
                    vq=collect_period(test,t,'quant',float(vtarget)/100,test_quant,'Validation')
                    dev=min(te,tq); val=min(ve,vq)
                    if dev<int(min_events):
                        # Remove partial rows so a rejected stock cannot contaminate aggregates.
                        train_early[:]=[r for r in train_early if r['Ticker']!=t]; train_quant[:]=[r for r in train_quant if r['Ticker']!=t]
                        test_early[:]=[r for r in test_early if r['Ticker']!=t]; test_quant[:]=[r for r in test_quant if r['Ticker']!=t]
                        skipped.append((t,f"only {dev} discovery target events")); continue
                    if val<1:
                        train_early[:]=[r for r in train_early if r['Ticker']!=t]; train_quant[:]=[r for r in train_quant if r['Ticker']!=t]
                        test_early[:]=[r for r in test_early if r['Ticker']!=t]; test_quant[:]=[r for r in test_quant if r['Ticker']!=t]
                        skipped.append((t,"no target events in unseen validation window")); continue
                    stock_rows.append({'Ticker':t,'Rows':len(f),'Discovery events':dev,'Validation events':val,'Split':'70/30'})
                except Exception as e:
                    skipped.append((t,f"calculation/data error: {type(e).__name__}"))
            bar.empty()

            def aggregate_oos(train_rows,test_rows):
                tr=pd.DataFrame(train_rows); va=pd.DataFrame(test_rows)
                if tr.empty or va.empty: return pd.DataFrame()
                out=[]
                comps=sorted(set(tr['Component']).intersection(set(va['Component'])))
                for comp in comps:
                    a=tr[tr.Component==comp].copy(); b=va[va.Component==comp].copy()
                    # Only compare stocks present in both phases.
                    common=sorted(set(a.Ticker).intersection(set(b.Ticker)))
                    a=a[a.Ticker.isin(common)]; b=b[b.Ticker.isin(common)]
                    tl=pd.to_numeric(a['Lift x'],errors='coerce'); vl=pd.to_numeric(b['Lift x'],errors='coerce')
                    valid=pd.DataFrame({'Ticker':b.Ticker.values,'Validation Lift':vl.values}).dropna()
                    n=len(valid); pos=int((valid['Validation Lift']>1.0).sum()) if n else 0
                    pos_rate=100*pos/n if n else 0
                    tmed=float(tl.median()) if tl.notna().any() else np.nan
                    vmed=float(vl.median()) if vl.notna().any() else np.nan
                    vmean=float(vl.mean()) if vl.notna().any() else np.nan
                    cov=float(pd.to_numeric(b['Coverage %'],errors='coerce').mean())
                    stab=float(pd.to_numeric(b['Stability %'],errors='coerce').mean())
                    repeat = np.isfinite(tmed) and np.isfinite(vmed) and tmed>1.0 and vmed>1.0
                    if n>=5 and repeat and vmed>=1.05 and pos_rate>=60:
                        verdict='OOS WINNER'
                    elif n>=5 and tmed>1.05 and (vmed<0.95 or pos_rate<=40):
                        verdict='FAILED OOS'
                    elif n>=5 and repeat:
                        verdict='PROMISING'
                    else:
                        verdict='MIXED / WEAK'
                    out.append({'Component':comp,'Discovery Median Lift':tmed,'Validation Median Lift':vmed,'Validation Mean Lift':vmean,'Validation Coverage %':cov,'Positive stocks':f'{pos}/{n}','Positive %':pos_rate,'Validation Stability %':stab,'Validation events':int(b.groupby('Ticker')['Events'].first().sum()),'Verdict':verdict})
                z=pd.DataFrame(out)
                if z.empty:return z
                rank={'OOS WINNER':0,'PROMISING':1,'MIXED / WEAK':2,'FAILED OOS':3}
                z['_r']=z.Verdict.map(rank).fillna(9)
                return z.sort_values(['_r','Validation Median Lift','Positive %'],ascending=[True,False,False]).drop(columns='_r').reset_index(drop=True)

            ea=aggregate_oos(train_early,test_early); qa=aggregate_oos(train_quant,test_quant)
            m1,m2,m3,m4=st.columns(4)
            m1.metric("Stocks validated",f"{len(stock_rows)}/{len(tickers)}")
            m2.metric("Skipped",len(skipped))
            m3.metric("Discovery events",sum(r['Discovery events'] for r in stock_rows))
            m4.metric("Validation events",sum(r['Validation events'] for r in stock_rows))

            for title,z in [("Early — unseen validation",ea),("Quant — unseen validation",qa)]:
                st.markdown(f"<div class='section'>{title}</div>",unsafe_allow_html=True)
                if z.empty:
                    st.warning("Not enough validation evidence for this section.")
                else:
                    show=z.copy()
                    for c in ['Discovery Median Lift','Validation Median Lift','Validation Mean Lift','Validation Coverage %','Positive %','Validation Stability %']:
                        show[c]=pd.to_numeric(show[c],errors='coerce').round(2)
                    st.dataframe(show,use_container_width=True,hide_index=True)
                    winners=z[z.Verdict=='OOS WINNER']
                    if not winners.empty:
                        st.success("Out-of-sample winners: "+', '.join(winners.Component.head(5).tolist()))
                    failed=z[z.Verdict=='FAILED OOS']
                    if not failed.empty:
                        st.warning("Looked good in discovery but failed unseen validation: "+', '.join(failed.Component.head(5).tolist()))

            st.markdown("<div class='section'>Stock sample audit</div>",unsafe_allow_html=True)
            ...
            st.info("V5.6 gate: only components that stay positive in the unseen 30% window across many stocks should be considered for future weight changes. This version still does not change live weights.")
