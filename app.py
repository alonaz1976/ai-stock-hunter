...
APP_VERSION = "5.8.3"
APP_BUILD_ID = "V583-ENTRY-COMP-20260914-A"
...
st.set_page_config(page_title="AI Stock Hunter — V5.8.3",page_icon="📈",layout="wide",initial_sidebar_state="collapsed")
st.markdown("""<style>
...
st.markdown("""<div class='hero'><span class='badge'>V5.8.3 • UNIQUE LIFT • COMPONENT OVERLAP • ENTRY VALIDATION</span><div class='hero-title'>📈 AI Stock Hunter<br>V5.8.3</div><div class='hero-sub'>Smart Money • Early Accumulation • Precision Entry • Walk-forward Backtest • Confidence Engine</div></div>""",unsafe_allow_html=True)
...
def chart(df,title,layers=None,show_rsi=False,show_macd=False):
    ...
    fig.update_layout(
        ...
        uirevision="stable-stock-chart-v580",
        dragmode="pan",
    )
...
def _entry_component_signals(r, static_quant=np.nan, dynamic_quant=np.nan):
    """Binary research signals for Entry Component Validation.
    Core signals mirror the live Entry Score formula. Smart-money candidates reuse
    the existing Early/Quant component definitions so the test does not invent new thresholds.
    """
    p=float(r.get('Close',np.nan)); vw=float(r.get('vwap',np.nan)); e9=float(r.get('ema9',np.nan)); e20=float(r.get('ema20',np.nan))
    vr=float(r.get('volume_ratio',np.nan)); mh=float(r.get('macd_hist',np.nan)); rv=float(r.get('rsi14',np.nan))
    core={
        'Core: Price ≥ VWAP': bool(np.isfinite(p) and np.isfinite(vw) and p>=vw),
        'Core: EMA9 ≥ EMA20': bool(np.isfinite(e9) and np.isfinite(e20) and e9>=e20),
        'Core: Volume Ratio ≥ 1.15': bool(np.isfinite(vr) and vr>=1.15),
        'Core: MACD histogram > 0': bool(np.isfinite(mh) and mh>0),
        'Core: RSI 50–70': bool(np.isfinite(rv) and 50<=rv<=70),
        'Core: Static Quant ≥ 66': bool(np.isfinite(static_quant) and float(static_quant)>=66),
        'Core: Dynamic Quant ≥ 66': bool(np.isfinite(dynamic_quant) and float(dynamic_quant)>=66),
    }
    ecom={n:float(pts)>0 for n,pts,mx in early_score_row(r)[1]}
    qcom={n:float(pts)>0 for n,pts,mx in score_row(r,0)[1]}
    candidates={
        'Candidate: Early RVOL': ecom.get('Early RVOL',False),
        'Candidate: Volume acceleration': qcom.get('Volume acceleration',False),
        'Candidate: Liquidity': qcom.get('Liquidity',False),
        'Candidate: OBV accumulation': ecom.get('OBV accumulation',False),
        'Candidate: CMF money flow': ecom.get('CMF money flow',False),
        'Candidate: Accumulation/Distribution': ecom.get('Accumulation/Distribution',False),
    }
    return {**core,**candidates}


def _aggregate_entry_components(component_rows):
    z=pd.DataFrame(component_rows)
    if z.empty:return pd.DataFrame()
    signal_cols=[c for c in z.columns if c.startswith('Core: ') or c.startswith('Candidate: ')]
    base=float(z['Hit'].mean()) if len(z) else np.nan
    out=[]
    for c in signal_cols:
        active=z[z[c].astype(bool)]; inactive=z[~z[c].astype(bool)]
        if active.empty:continue
        ahr=float(active.Hit.mean()); ihr=float(inactive.Hit.mean()) if not inactive.empty else np.nan
        stock_rates=active.groupby('Ticker')['Hit'].mean()
        fold_rates=active.groupby('Fold')['Hit'].mean()
        pos_stocks=100*float((stock_rates>base).mean()) if len(stock_rates) and np.isfinite(base) else np.nan
        pos_folds=100*float((fold_rates>base).mean()) if len(fold_rates) and np.isfinite(base) else np.nan
        out.append({
            'Component':c,'Active rows':len(active),'Coverage %':100*len(active)/len(z),
            'Hit Rate %':100*ahr,'Lift vs baseline':ahr/base if np.isfinite(base) and base>0 else np.nan,
            'Inactive Hit %':100*ihr if np.isfinite(ihr) else np.nan,
            'Unique Lift x':ahr/ihr if np.isfinite(ihr) and ihr>0 else np.nan,
            'Incremental hit pp':100*(ahr-ihr) if np.isfinite(ihr) else np.nan,
            'Avg Forward Return %':100*float(active.ForwardReturn.mean()),
            'Avg Drawdown %':100*float(active.Drawdown.mean()),
            'Positive stocks %':pos_stocks,'Positive folds %':pos_folds,
            'Stocks':int(active.Ticker.nunique()),'Folds':int(active.Fold.nunique()),
        })
    return pd.DataFrame(out).sort_values(['Unique Lift x','Lift vs baseline','Positive stocks %'],ascending=False,na_position='last').reset_index(drop=True)
...
tab1,tab2,tab3,tab4,tab5=st.tabs(["◉ Analyze","⌁ Scanner","▦ Backtest","◎ Validate","↗ Entry Validation"])
with tab1:
    ...
    if missing_engine_api:
        st.error("Quant engine is incompatible. Missing: " + ", ".join(missing_engine_api) + ". Upload all 4 files from the V5.8.1 ZIP.")
        st.stop()
    ...
    if st.button("Run V5.8.1 Dynamic analysis"):
...
with tab4:
    st.markdown("<div class='section'>V5.8.3 Walk-Forward Validation Lab</div>", unsafe_allow_html=True)
    ...
    if st.button("Run V5.8.3 Walk-Forward Validation", key="run_multi_validation"):
        tickers=[]
        ...
        if len(tickers)<3:
            ...
            early_rows=[]; quant_rows=[]; stock_rows=[]; skipped=[]; overlap_frames=[]; unique_frames=[]
            bar=st.progress(0, text="Starting V5.8.3 walk-forward validation...")
            ...
            for idx,t in enumerate(tickers,1):
                ...
                try:
                    ...
                    accepted=0; tr_events=0; va_events=0; er=[]; qr=[]; accepted_fold_ranges=[]
                    for fold,(train_end,test_end) in enumerate(fold_specs,1):
                        ...
                        accepted+=1; accepted_fold_ranges.append((fold,i1,i2)); tr_events+=tev; va_events+=vev; er.extend(ter+ver); qr.extend(tqr+vqr)
                    ...
                    # V5.8.3 Unique-lift sample: use ONLY rows inside accepted chronological
                    # validation blocks.  The outcome is whether a +target close-to-close
                    # event occurs within the next 1–3 trading days, matching the Early lab horizon.
                    for fold,i1,i2 in accepted_fold_ranges:
                        closes=f['Close'].to_numpy(dtype='float64')
                        for j in range(i1,min(i2,len(f)-3)):
                            r=f.iloc[j]
                            ecom={name:(float(pts)/float(mx) if mx else 0.0) for name,pts,mx in early_score_row(r)[1]}
                            qcom={name:(float(pts)/float(mx) if mx else 0.0) for name,pts,mx in score_row(r,0)[1]}
                            future=[closes[k]/closes[j]-1.0 for k in range(j+1,min(j+4,len(f))) if np.isfinite(closes[k]) and np.isfinite(closes[j]) and closes[j]!=0]
                            hit=1 if future and max(future)>=float(vtarget)/100 else 0
                            unique_frames.append({'Ticker':t,'Fold':fold,'Hit':hit,
                                'Early: Early RVOL':ecom.get('Early RVOL',0.0),
                                'Quant: Volume acceleration':qcom.get('Volume acceleration',0.0),
                                'Quant: Liquidity':qcom.get('Liquidity',0.0),
                                'Quant: ADX trend strength':qcom.get('ADX trend strength',0.0)})
            ...
            st.markdown("<div class='section'>Unique Lift / Ablation</div>",unsafe_allow_html=True)
            st.caption("V5.8.3 asks a stricter question: after the other highlighted signals are already present, does each component still add predictive information on unseen walk-forward rows? Live weights are still NOT changed.")
            if unique_frames:
                uz=pd.DataFrame(unique_frames)
                focus_u=['Early: Early RVOL','Quant: Volume acceleration','Quant: Liquidity','Quant: ADX trend strength']
                urows=[]
                base=float(uz['Hit'].mean()) if len(uz) else np.nan
                for comp in focus_u:
                    others=[c for c in focus_u if c!=comp]
                    active=uz[comp]>0
                    # Control for the other highlighted signals: compare rows where at least
                    # one other focus component is active, then toggle this component on/off.
                    control=uz[others].gt(0).any(axis=1)
                    on=uz[control & active]; off=uz[control & ~active]
                    hr_on=float(on['Hit'].mean()) if len(on) else np.nan
                    hr_off=float(off['Hit'].mean()) if len(off) else np.nan
                    unique=(hr_on/hr_off) if np.isfinite(hr_on) and np.isfinite(hr_off) and hr_off>0 else np.nan
                    delta=(hr_on-hr_off)*100 if np.isfinite(hr_on) and np.isfinite(hr_off) else np.nan
                    stocks_on=int(on['Ticker'].nunique()) if len(on) else 0
                    folds_on=int(on[['Ticker','Fold']].drop_duplicates().shape[0]) if len(on) else 0
                    if len(on)>=40 and len(off)>=40 and stocks_on>=8 and np.isfinite(unique) and unique>=1.10 and delta>=2.0:
                        verdict='UNIQUE CONTRIBUTOR'
                    elif len(on)>=25 and len(off)>=25 and np.isfinite(unique) and unique>=1.03:
                        verdict='PROMISING UNIQUE EDGE'
                    elif np.isfinite(unique) and unique<0.97:
                        verdict='NO UNIQUE EDGE'
                    else:
                        verdict='INCONCLUSIVE'
                    urows.append({'Component':comp.replace('Early: ','').replace('Quant: ',''),
                                  'Active rows':len(on),'Control rows':len(off),'Stocks active':stocks_on,
                                  'Stock/folds active':folds_on,'Hit when active %':100*hr_on if np.isfinite(hr_on) else np.nan,
                                  'Hit control %':100*hr_off if np.isfinite(hr_off) else np.nan,
                                  'Unique Lift x':unique,'Incremental hit pp':delta,'Verdict':verdict})
                ut=pd.DataFrame(urows).sort_values(['Unique Lift x','Incremental hit pp'],ascending=False,na_position='last').reset_index(drop=True)
                ush=ut.copy()
                for c in ['Hit when active %','Hit control %','Unique Lift x','Incremental hit pp']:
                    ush[c]=pd.to_numeric(ush[c],errors='coerce').round(2)
                st.dataframe(ush,use_container_width=True,hide_index=True)
                winners=ut[ut.Verdict=='UNIQUE CONTRIBUTOR']
                if not winners.empty:
                    st.success("Unique contributors on unseen rows: "+', '.join(winners.Component.tolist()))
                weak=ut[ut.Verdict=='NO UNIQUE EDGE']
                if not weak.empty:
                    st.warning("No unique edge after controlling for the other highlighted signals: "+', '.join(weak.Component.tolist()))
                st.caption(f"Validation-row baseline hit rate: {base*100:.2f}% • Outcome: target reached by close within the next 1–3 trading days. This is an ablation-style diagnostic, not a causal proof.")
            else:
                st.info("No usable unseen validation-row sample was available for Unique Lift with these settings.")
            ...
            st.info("V5.8.3 gate: do not change live weights yet. A component should survive multiple unseen chronological folds and many stocks before V5.8 weight optimization.")

    st.markdown("<div class='section'>V5.8 Liquidity / validation notes</div>",unsafe_allow_html=True)
...
with tab5:
    ...
    if st.button("Run Entry Validation on Validation 50",key="run_entry_validation"):
        ...
        fold_specs=[(.40,.55),(.55,.70),(.70,.85),(.85,1.00)]
        rows=[]; component_rows=[]; audit=[]; skipped=[]
            ...
            try:
                ...
                for fold,(train_end,test_end) in enumerate(fold_specs,1):
                    ...
                    for j in range(len(test)):
                        ...
                        sigs=_entry_component_signals(r,sq,dq)
                        component_rows.append({**common,**sigs})
        ...
        if not rows:
            ...
            st.markdown("#### Entry Component Validation — core formula + Smart Money candidates")
            st.caption("Breaks the Entry engine into its live core conditions and research candidates. Unique Lift compares target hit rate when a component is active versus inactive on the same unseen walk-forward rows. No Entry weights are changed in V5.8.3.")
            comp_table=_aggregate_entry_components(component_rows)
            if comp_table.empty:
                st.info("No usable Entry component evidence was produced for these settings.")
            else:
                cshow=comp_table.copy()
                for c in ['Coverage %','Hit Rate %','Lift vs baseline','Inactive Hit %','Unique Lift x','Incremental hit pp','Avg Forward Return %','Avg Drawdown %','Positive stocks %','Positive folds %']:
                    if c in cshow: cshow[c]=pd.to_numeric(cshow[c],errors='coerce').round(2)
                st.dataframe(cshow,use_container_width=True,hide_index=True)
                strong=comp_table[(comp_table['Unique Lift x']>=1.05)&(comp_table['Incremental hit pp']>=2)&(comp_table['Positive stocks %']>=55)&(comp_table['Folds']>=3)]
                if not strong.empty:
                    st.success("Entry component candidates with useful unseen contribution: "+', '.join(strong.Component.tolist()))
                weak=comp_table[(comp_table['Unique Lift x']<1.0)&(comp_table['Active rows']>=50)]
                if not weak.empty:
                    st.warning("Entry components that were weaker when active in this run: "+', '.join(weak.Component.tolist()))
                st.caption("Interpretation gate: prefer components with Unique Lift > 1, positive incremental hit rate, broad stock coverage and consistency across folds. Correlated signals still need the overlap test before both receive large weights.")
            ...
            if skipped:
                with st.expander(f"Skipped stocks ({len(skipped)})"):
                    st.dataframe(pd.DataFrame(skipped,columns=['Ticker','Reason']),use_container_width=True,hide_index=True)
            st.caption("This V5.8 test validates the current Entry Score formula without changing it. Intraday 15m/1H history is provider-limited, so the multi-year 50-stock walk-forward comparison uses daily historical features for a consistent apples-to-apples test.")
